...
import '../blocs/category_tab9_bloc.dart';

class CategoryTab9 extends StatefulWidget {
  final int categoryId;
  final GlobalKey<ScaffoldState> scaffoldKey;

  CategoryTab9({Key? key, required this.categoryId, required this.scaffoldKey})
      : super(key: key);

  @override
  _CategoryTab9State createState() => _CategoryTab9State();
}

class _CategoryTab9State extends State<CategoryTab9> {
  @override
  void initState() {
    super.initState();
    if (this.mounted) {
      if (context.read<CategoryTab9Bloc>().articles.isEmpty) {
        context.read<CategoryTab9Bloc>().fetchData(widget.categoryId, mounted);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    final cb = context.watch<CategoryTab9Bloc>();
